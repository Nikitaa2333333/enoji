...
def has_underline(page, line_words):
    ...
    for obj in objs:
        ...
        if search_min_y <= o_top <= search_max_y:
            # Учитываем, что линии в PDF могут быть чуть шире или уже текста
...
def render_html(lines, country_name, slug):
    ...
    def flush_table():
        ...
        if table_buf:
            ...
            main_c += '<div class="grid grid-cols-1 md:grid-cols-2 gap-8 my-10">\n'
            for col in [c1, c2]:
                cl = [p for p in col if p.strip()]
                if not cl: continue
                hdr = re.sub(r'</?b>', '', cl[0]); bdy = "<br>".join(cl[1:])
                bdy = bdy.replace("<b>", '<span class="font-bold text-black">').replace("</b>", '</span>')
                main_c += f'  <div class="bg-gray-50/50 p-8 rounded-3xl border border-gray-100 h-full shadow-sm"> <div class="text-xl font-bold mb-4 text-black">{hdr}</div> <div class="text-lg text-black/70 leading-relaxed font-manrope">{bdy}</div> </div>\n'
            main_c += '</div>\n'; table_buf = []
...
def main():
    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            html = render_html(lines, c_name, slug)
